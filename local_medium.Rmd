...
run_local_model<-function(theta1,theta2){
  
  if(theta1>2 & theta2>2)
  {
    sample<-rnorm(n=100,mean=theta1,sd=theta2)
...
total_data<-
  data.frame(
    theta1 = runif(RUNS_TO_MAKE,min = 0.1,max=5),
    theta2 = runif(RUNS_TO_MAKE,min = 0.1,max=5)
    #theta2 = sample(c(1,1000,resample=TRUE,size=1000))
  ) 

total_data<-pmap_df(total_data,run_local_model) %>% cbind(total_data)
