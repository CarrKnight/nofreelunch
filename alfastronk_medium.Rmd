...
library(tidyverse)
```
Rubio and Johansen (2013) : A simple approach to maximum intractable likelihood estimation
But we aren't using complex numbers. In theory regressions ought to be able to deal with complex numbers, but in practice I am not sure random forest can.

```{r dataprep}
RUNS_TO_MAKE<-MEDIUM_DATA_SET_SIZE
SAMPLE_SIZE<-1096 # days between Jan 1 2009 and Jan 1 2012 
library(stabledist)


# example from: https://projecteuclid.org/download/pdfview_1/euclid.ejs/1371649229
alfastronk<-function(alpha,mu,sigma){
  #notice that mu is delta and sigma is gamma in R terms
  simulation <- rstable(n = SAMPLE_SIZE,alpha = alpha,beta=0,gamma=sigma,delta=mu)
  return(
...
total_data<-
  data.frame(
    alpha = runif(RUNS_TO_MAKE,1,2),
    mu= runif(RUNS_TO_MAKE,-0.1,0.1),
    sigma= runif(RUNS_TO_MAKE,0.0035,0.0125)

  )
total_data<-pmap_df(total_data,alfastronk) %>% cbind(total_data)


param_names<-c("alpha","mu","sigma")
