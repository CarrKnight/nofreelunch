...
run_fat_model<-function(weight1,weight2){
  # print(mean)
  #  print(sd)
  ...
  return(
    data.frame(
      total = weight1+weight2 + rnorm(sd=1,n=1,mean=0)
    )
  )
}

param_names<-c("weight1","weight2")


total_data<-
  data.frame(
    weight1 = runif(RUNS_TO_MAKE,80,150),
    weight2= runif(RUNS_TO_MAKE,80,150)

  )
total_data<-pmap_df(total_data,run_fat_model) %>% cbind(total_data)
...
SABC won't work if of degree 1 and only 1 summary statistic
