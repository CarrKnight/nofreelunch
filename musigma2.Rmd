...
```{r dataprep}
# first example, musigma2
data(musigma2)
musigma_df<-cbind(data.frame(par.sim),data.frame(stat.sim))

# pre-transform sigma2
total_data<-musigma_df %>%
  mutate(sigma2 = log(sigma2))


param_names<-c("mu","sigma2")
...
```{r loess}
loess_cross<-
  cross_validate_loess(total_data,ngroup=5,
                    parameter_colnames = param_names,x_names=x_names)
