...
dr_sumstat<-read_csv("./data/mlevel/mlevel.csv",
                     col_names=TRUE) 
...
# I add to add a "row.number" column name to the file to read this correctly
dr_parameters<-
  read_csv("./data/mlevel/params_mlevel.csv",
                     col_names=TRUE) %>%
...
total_data<-
  total_data %>% select_if(function(x){var(x)>0})
...
loclin_cross<-
  cross_validate_abc(total_data,ngroup=5,method="loclinear",
                     ...
                     semiauto = TRUE,satr=list(function(x){outer(x,Y=1,"^")}))
