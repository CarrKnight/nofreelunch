...
From Wilkinson's tutorial NIPS 2013
https://www.youtube.com/watch?v=fYu-asA-qMs&t=1s
...
toy_model<-function(theta){
  

  rnorm(1, 2*(theta+2)*theta*(theta-2),sd=sqrt(0.1 + theta^2))
}


run_toy_model<-function(theta){
  simulation<-
    toy_model(theta=theta)
   return(
    data.frame(
      s1=simulation[1]
    )
...
total_data<-
  data.frame(
    theta = runif(RUNS_TO_MAKE,-10,+10)
