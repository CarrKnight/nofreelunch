...
Ma2 from Creel's Neural nets for indirect inference
...
run_ma<-function(ma1,ma2){
  
 observations<-arima.sim(n=NUM_OBSERVATIONS,
                         list(ma=c(ma1,ma2)),sd=1)
  
  return(
    arima(x = observations, order = c(10,0,0), include.mean = F, method="CSS")  %>% 
      coef() %>% as.list() %>% 
      as.data.frame()
  )
}
...
total_data<-
  data.frame(
    ma1 = runif(100000,min = -2,max= +2),
    ma2 = runif(100000,min = -1,max= +1)
  ) %>%
  filter(ma1+ma2>-1) %>%
  filter(ma1-ma2>-1) %>%
  sample_n(RUNS_TO_MAKE)

total_data<-pmap_df(total_data,run_ma) %>% cbind(total_data)

param_names<-c("ma1","ma2")
